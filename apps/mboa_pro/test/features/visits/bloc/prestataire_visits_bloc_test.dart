...
class MockPrestataireVisitRepository extends Mock
    implements PrestataireVisitRepository {}
...
void main() {
  ...
  blocTest<PrestataireVisitsBloc, PrestataireVisitsState>(
    ...
    setUp: () =>
        when(() => repository.confirm('v-1')).thenAnswer((_) async {}),
    build: build,
  ...
  blocTest<PrestataireVisitsBloc, PrestataireVisitsState>(
    ...
    setUp: () =>
        when(() => repository.decline('v-1')).thenAnswer((_) async {}),
    build: build,
  ...
  blocTest<PrestataireVisitsBloc, PrestataireVisitsState>(
    ...
    setUp: () =>
        when(() => repository.confirm('v-1')).thenThrow(Exception('409')),
    build: build,
    act: (bloc) => bloc.add(const VisitRequestConfirmed('v-1')),
    expect: () => [
      const PrestataireVisitsBusy('v-1'),
