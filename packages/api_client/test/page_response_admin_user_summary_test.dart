import 'package:test/test.dart';
import 'package:api_client/api_client.dart';

// tests for PageResponseAdminUserSummary
void main() {
  final instance = PageResponseAdminUserSummaryBuilder();
  // TODO add properties to the builder and call build()

  group(PageResponseAdminUserSummary, () {
    // BuiltList<AdminUserSummary> content
    test('to test the property `content`', () async {
      // TODO
    });

    // int page
    test('to test the property `page`', () async {
      // TODO
    });

    // int size
    test('to test the property `size`', () async {
      // TODO
    });

    // int totalElements
    test('to test the property `totalElements`', () async {
      // TODO
    });

    // int totalPages
    test('to test the property `totalPages`', () async {
      // TODO
    });

    // bool last
    test('to test the property `last`', () async {
      // TODO
    });

  });
}
